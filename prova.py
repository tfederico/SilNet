...
device = "cuda:0"
model = SilNet()
model.load_state_dict(torch.load("model/silnet.pth"))
model.to(device)

model.eval()   # Set model to the evaluation mode

#for inputs, labels in dataloaders[phases[2]]:
inputs = Image.open("feature1.jpg")
labels = Image.open("mask1.jpg")
inputs = transforms["feature"](inputs)
inputs = inputs.unsqueeze(0)
labels = transforms["target"](labels)
